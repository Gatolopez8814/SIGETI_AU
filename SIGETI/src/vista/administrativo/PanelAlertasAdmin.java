...
public class PanelAlertasAdmin extends javax.swing.JPanel {

    private PanelAlertasAdmin() {
        initComponents();
    }//----------------------------------------------------------------------------- FIN Constructor()

    public static PanelAlertasAdmin obtenerInstancia() {//para garantizar hay solo una instancia
        if (instancia == null) {
            instancia = new PanelAlertasAdmin();
        }
        instancia.ajustarEventos();
        instancia.limpiarCampos();
        instancia.ocultarComponentes();
        instancia.llenarBandeja();
        return instancia;
    ...
    private void llenarBandeja() {
        this.cantidad.setText(String.valueOf(Controlador.obtenerInstancia().numeroAlerta()));
        modelAux = (DefaultTableModel) tablaTickets.getModel();
        while (modelAux.getRowCount() > 0) {
            modelAux.removeRow(0);
        }
        ArrayList<Ticket> aux = Controlador.obtenerInstancia().ticketsAlertaAdmin();
        if (aux.isEmpty()) {
        } else {
            int i = 0;
            while (i < aux.size()) {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Los siguientes tickets no fueron resueltos en el tiempo estimado ");
        ...
        jPanelResponsableLayout.setHorizontalGroup(
            ...
            .addGroup(jPanelResponsableLayout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(jComboUsuarios, javax.swing.GroupLayout.PREFERRED_SIZE, 171, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        );
...
//        jTextAnnoDesde.setText("");
    ...
    private static PanelAlertasAdmin instancia = null;
